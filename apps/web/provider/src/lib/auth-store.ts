  ...
  persist(
    (set) => ({
      ...
      login: async (phoneOrEmail, password) => {
        ...
        const d = r.data?.data;
        localStorage.setItem('nitaq_carrier_token', d.accessToken);
        localStorage.setItem('nitaq_carrier_refresh', d.refreshToken);
        set({ user: d.user });
      },
      logout: async () => {
        const rt = localStorage.getItem('nitaq_carrier_refresh');
