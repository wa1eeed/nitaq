...
┌──────────────────────────────────────────────────────────────────┐
│                         العميل النهائي                            │
│  Landing  │   Admin   │   Client   │  Provider   │   Mobile (TBD) │
│  :3000    │   :3001   │   :3002    │   :3003     │                │
└──────────────────────────────────────────────────────────────────┘
                                │
                                │ HTTPS / WSS
...
│                                                                  │
│  Auth · Users · Companies · Orders · Bids · Services · Payments │
...
│       └── carrier/                       # :3003 — تطبيق مقدّم الخدمة
│
...
| **Companies** | `modules/companies` | شركات العملاء ومقدّمي الخدمات، KYC، اعتماد |
| **Orders** | `modules/orders` | إنشاء، نشر، تأكيد، إلغاء، تحديث حالة |
| **Bids** | `modules/bids` | تقديم عرض، قبول، رفض، سحب |
| **Services** | `modules/services` | خدمات، موظفون، تحديث موقع |
...
| **Admin** | :3001 | إدارة · الخدمات · المالية · النظام · القانوني · الإعدادات | 17 صفحة |
| **Client** | :3002 | منصة العميل · المالية · الدعم · الإعدادات | 10 صفحات |
| **Provider** | :3003 | مقدّم الخدمة · الخدمات · الموظفون · المالية · الامتثال · الإعدادات | 14 صفحة |
...
#### 👤 Users & Companies
- **`User`** — حسابات المستخدمين (admin/client/provider/employee)
- **`Company`** — كيانات الشركات (عميل أو مقدّم خدمة)
- **`KYCDocument`** — مستندات الاعتماد (CR/VAT/Zakat/تأمين/رخصة)
- **`EmployeeProfile`** — بيانات الموظفين (رقم الهوية، الحالة، التعيينات)
- **`RefreshToken`** — refresh tokens للـ JWT
- **`ApiKey`** — مفاتيح API لـ integrations مستقبلية

#### 🛠️ Services
- **`Service`** — الخدمات (code, type, capacity, status)

#### 📦 Orders & Bids
- **`Order`** — الطلبات (mode: OPEN/DIRECT، 9 حالات)
- **`Bid`** — العروض (price, days, notes, status)
- **`OrderService`** — ربط الخدمة بالطلب
- **`OrderEmployee`** — ربط الموظف بالطلب

#### 📍 Tracking
- **`TrackingEvent`** — events الـ Timeline لكل طلب
- **`LocationHistory`** — تاريخ المواقع للموظفين
...
#### ⚖️ Disputes & Reviews
- **`Dispute`** — نزاعات (raisedBy CLIENT/PROVIDER)
...
### 4.2 Enums (13)
`UserRole`, `CompanyType`, `CompanyStatus`, `KYCStatus`, `OrderStatus`, `ServiceType`, `CargoType`, `BidStatus`, `PaymentStatus`, `InvoiceStatus`, `NotificationType`, `DisputeStatus`, `EmployeeStatus`
...
Company (provider) ──1:N── Service
Company (provider) ──1:N── EmployeeProfile
Company (client) ──1:N── Order
Order ──N:1── Company (provider, optional)
Order ──1:N── Bid
Order ──1:N── TrackingEvent
Order ──1:1── Payment
Order ──1:1── Invoice
Order ──1:N── Dispute
Employee ──1:N── LocationHistory
...
[العميل]
    ...
    │   └─ DIRECT mode    → PUBLISHED (مرسل لمقدّم خدمة محدد)
    │
[مقدّمو الخدمات]
    │
...
[مقدّم الخدمة]
    ...
    ├─ يبدأ التنفيذ → IN_TRANSIT
    │   (تتبع لحظي عبر Socket.io)
    ├─ يُنجز ويُسلّم → DELIVERED
    │
[العميل]
    │
    ├─ يؤكّد الاستلام → COMPLETED
    │   (Escrow released → provider wallet -عمولة -VAT)
    │
    OR
    │
...
| الحالة | إلغاء العميل | إلغاء مقدّم الخدمة |
|---|---|---|
| `DRAFT`, `PUBLISHED`, `BIDDING` | مجاني | — |
| `ASSIGNED`, `CONFIRMED` | **رسوم 10%** لمقدّم الخدمة | غرامة على تقييمه |
...
2. مقدّم الخدمة يُنجز
   └─ status = DELIVERED

3. العميل يؤكّد (أو 72 ساعة تمر)
   ├─ المنصة تخصم 8% عمولة
   ├─ تُحوّل (92% - VAT على العمولة) لمحفظة مقدّم الخدمة
   ├─ تُصدر فاتورة للعميل
   └─ Status = COMPLETED

4. مقدّم الخدمة يطلب سحب رصيد محفظته
...
| الحالة | التعويض |
|---|---|
| تأخر مقدّم الخدمة > 4 ساعات عن موعد البدء | 5% خصم تلقائي للعميل |
| خلل في الخدمة | نزاع → admin يقرّر النسبة |
| إلغاء مقدّم الخدمة بعد التأكيد | استرداد كامل + غرامة على مقدّم الخدمة |
| إلغاء العميل بعد ASSIGNED | 10% لمقدّم الخدمة |

### 5.5 Workflow Engine ✅ مُطبَّق (v0.6.0)

كل قواعد الـ workflow في ملف واحد: **`packages/shared-utils/src/workflow-engine.ts`**. أي UI surface (client/provider/admin) يستهلك الـ helpers بدل ما يعيد كتابة المنطق.
...
// Escrow
escrowBreakdown(total)             → { total, commission, providerAmount }
...
|---|---|---|
| `ORDER_PUBLISHED` | provider | 1 |
| `BID_SUBMITTED` | client | 1 |
| `BID_ACCEPTED` | provider (winner+losers), admin | 3 |
| `ORDER_CONFIRMED` | client, admin | 2 |
| `ORDER_IN_TRANSIT` | client | 1 |
| `ORDER_DELIVERED` | client, admin | 2 |
| `ESCROW_RELEASED` | provider, client | 2 |
| `ORDER_CANCELLED` | client, provider, admin | 3 |
| `DISPUTE_OPENED` | admin, client, provider | 3 |
| `DISPUTE_RESOLVED` | client, provider | 2 |
...
- **Client app** (`apps/web/client/src/components/escrow-countdown.tsx`):
  ...
  - تفصيل: إجمالي / عمولة 8% / صافي لمقدّم الخدمة
  - زر "تأكيد الآن" للإفراج الفوري
- **Provider app** (`apps/web/provider/src/components/escrow-countdown.tsx`):
...
#### من Client/Provider → Server
- `subscribe:order` — الاشتراك في تحديثات طلب
- `subscribe:tracking` — تتبّع موظف
...
- `tracking:location_update` — موقع جديد للموظف
...
| `useBrandsStore` | `stores/brands-store.ts` | براندات وموديلات الخدمات | ✅ |
| `useServiceTypesStore` | `stores/service-types-store.ts` | أنواع الخدمات + صورها | ✅ (versioned) |
...
### 8.2 SWR
- يُستخدم لاستدعاءات الـ API (orders, bids, services, ...)
...
| **CORS** | Allowlist صارمة (لا wildcard) — `localhost` للتطوير + `nitaq.sa` و `*.nitaq.sa` للإنتاج |
...
└─────────────────────────────────────────────────────────┘
┌─────────────────────────────────────────────────────────┐
│        Admin / Client / Provider (Cookie-based)          │
...
└─────────────────────────────────────────────────────────┘
   ...
   `messages/{ar|en}/*.json`  (10 files × 2 locales = 468 keys)
       │
       ├─ common · auth · orders · services · payments
       ├─ notifications · admin · client · provider
       └─ landing
```
...
└── apps/web/<app>/
    ...
    │   └── components/
    │       ├── language-selector.tsx        # admin/client/provider
...
2. i18n.readLocaleFromHeaders():
     - cookies().get('nitaq-locale') → 'en' (if set)
...
5. `setLocaleAction('en')` server action:
     - cookies().set('nitaq-locale', 'en', { maxAge: 1y })
...
| `services.json` | 40+ | أنواع الخدمات، الموظفون، الوثائق |
...
| `provider.json` | 50+ | فرص، عروض، وثائق، خدمات |
...
- [ ] لغات إضافية (أوردو، هندي للموظفين الدوليين)
...
حسابات الشركات على المنصة (CLIENT و PROVIDER) مشتركة بين أكثر من شخص: مالك، مديرون، مشرف عمليات، محاسب. كل واحد منهم يحتاج صلاحيات مختلفة. نظام الـ Team Members يتيح للشركة دعوة أعضاء، تعيين دور لكل واحد، وإيقاف/حذف من تشاء.
...
enum CompanyRole {
  ...
  DISPATCH  // (مقدّم خدمة) — مشرف خدمات
  FINANCE   // محاسب (قراءة فقط على المال + export)
}
```

| Role | للعميل | لمقدّم الخدمة |
|---|:---:|:---:|
| OWNER    | ✅ | ✅ |
| ADMIN    | ✅ | ✅ |
| STAFF    | ✅ | — |
...
`CLIENT_ROLE_PERMISSIONS` و `PROVIDER_ROLE_PERMISSIONS`.
...
| **Provider** | `/settings` → تبويب "الفريق" | CRUD كامل |
| **Admin** | `/companies/[id]` (قسم Team) | Read-only + Suspend/Remove فقط |

ملف `TeamTab` مكوّن مُعاد استخدامه في Client + Provider:
`apps/web/{client,provider}/src/components/settings/team-tab.tsx`
...
│  Staging      →  .env.staging      →  api.staging.nitaq.sa  │
│  Production   →  .env.production   →  api.nitaq.sa          │
└─────────────────────────────────────────────────────────────┘
                          │
                          ▼
...
```
┌─ Setting ────────────────────┐
│  platform_name_ar    "نيطاق"  │  ← اسم المنصة بالعربية
│  platform_name_en    "Nitaq" │  ← اسم المنصة بالإنجليزية
...
| **Service Types** | `/admin/settings/service-types` | 10 أنواع بـ emoji + flags | اختيار نوع الخدمة في الـ Wizard |
| **SMS Templates** | `/admin/settings/sms-templates` | 4 قوالب (invitation, OTP, order, bid) | نصوص الإشعارات الصادرة |
| **Service Types** | `/admin/settings/brands` | 5+ أنواع | dropdown إضافة خدمة في Provider |
...
- **`RouteMap`** (`apps/web/client/src/components/route-map.tsx`) — موجود من قبل، يعرض المسار origin→destination + employee progress
